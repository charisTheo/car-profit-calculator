...
function App() {
  ...
  const [profitPercentage, setProfitPercentage] = useState(
    initialOptions.profitPercentage,
  );
  const [shippingCosts, setShippingCosts] = useState(
    initialOptions.shippingCosts,
  );
  ...
  const [isVATQualified, setIsVATQualified] = useState(
    initialOptions.isVATQualified,
  );
  const [includeAuctionFees, setIncludeAuctionFees] = useState(
    initialOptions.includeAuctionFees,
  );
  const [auctionSite, setAuctionSite] = useState(initialOptions.auctionSite);
  const [importLocation, setImportLocation] = useState(
    initialOptions.importLocation,
  );
  ...
  const exchangeRate = useExchangeRate(exchangeRateProvider, {
    from: "EUR",
    to: currency,
  });
  const japaneseExchangeRate = useExchangeRate(exchangeRateProvider, {
    from: "JPY",
    to: "EUR",
  });
        ...
        <Grid container spacing={2} justifyContent="center">
          ...
          <Grid size={{ xs: 12, md: 7 }}>
            <Paper elevation={3} sx={{ p: 2, borderRadius: 2, height: "100%" }}>
              ...
              <ResultRow
                label="Initial Car Price"
                value={calculations.initialPrice}
              />
              {calculations.ukReturnedVAT > 0 && (
                <ResultRow
                  label="UK Returned VAT (20%)"
                  value={calculations.ukReturnedVAT}
                />
              )}
              <ResultRow
                label="Shipping Costs"
                value={calculations.shippingCosts}
              />
              <ResultRow
                label={`Import Duties (${calculations.importDutyRate * 100}%)`}
                value={calculations.importDuties}
              />
                  ...
                  tooltipContent={
                    <Box>
                      <Typography variant="body2">
                        Service Fee:{" "}
                        {formatCurrency(
                          calculations.auctionFeeBreakdown.serviceFee,
                        )}
                      </Typography>
                      <Typography variant="body2">
                        Documentation Fee:{" "}
                        {formatCurrency(
                          calculations.auctionFeeBreakdown.documentationFee,
                        )}
                      </Typography>
                      <Typography variant="body2">
                        Transportation Fee:{" "}
                        {formatCurrency(
                          calculations.auctionFeeBreakdown.transportationFee,
                        )}
                      </Typography>
                    </Box>
                  }
              ...
              {calculations.bankTransferFees > 0 && (
                <TooltipBreakdownRow
                  ...
                  tooltipContent={
                    <Typography variant="body2">
                      Eurobank tariff (03/2025): outgoing payment in foreign
                      currency with debit in EUR -- 0.30% (min EUR 6, max EUR
                      500). Based on car price plus auction fees in EUR.
                      SHA/OUR, same-day, or other optional charges are not
                      included; the bank may pass on correspondent costs.
                    </Typography>
                  }
                />
              )}
              ...
              <ResultRow
                ...
                value={calculations.totalLandedCost}
                isBold
              />
              <ResultRow
                label={`VAT on Landed Cost (${calculations.totalLandedCost} x ${CY_VAT_RATE * 100}%)`}
                value={calculations.vatOnLandedCost}
              />
              <ResultRow
                label={`Additional VAT on Profit (${calculations.profit} x ${CY_VAT_RATE * 100}%)`}
                value={calculations.additionalVAT}
              />
              <ResultRow
                label="Total Costs"
              ...
              <ResultRow
                ...
                value={calculations.finalSalePrice}
                isFinal
                isHighlighted
              />
            </Paper>
